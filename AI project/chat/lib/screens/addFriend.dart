          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Text('No users.',style: TextStyle(
                  fontSize: 20.0,
                  color: Colors.black26
                ),):
                StreamBuilder(
                  stream: _firestore.collection('user').where('name',isEqualTo: userName.text).snapshots(),
                    ...
                    for(var us in data){
                      users.add(UserRequestTile(us.data()['name'],us.id, ''));
                    }

                    return users.isNotEmpty?Column(
                      children: users,
                    ):Text('No user Found. Try correcting username',style: TextStyle(
                        fontSize: 20.0,
                        color: Colors.black26
                    ),);
                    }
    ...
    return ListTile(
      ...
      trailing: IconButton(
          onPressed: ()async{
            ...
            await _firestore.collection('user').doc(widget.uid).collection('requests').doc().set({
